...
const root=path.resolve(__dirname,'../reflex-nano'),here=path.join(root,'benchmarks/lifecycle');
...
const command = ['nano', 'nano_react', 'nano_before'].includes(kind) ? [path.join(root,
  ...
  [python, kind === 'reflex' ? path.join(here,'reflex_server.py') : path.join(__dirname,'python_server.py')];
const hostPids=new Map();
function serverMemory(pid) {
  if(!hostPids.has(pid)) {
    ...
    for(const name of fs.readdirSync('/proc')) {
      ...
      try {
        if(fs.readlinkSync(`/proc/${name}/ns/pid`)!==ns)continue;
        const match=fs.readFileSync(`/proc/${name}/status`,'utf8').match(/^NSpid:\s+(.+)$/m);
        if(match&&Number(match[1].trim().split(/\s+/).at(-1))===pid) {hostPids.set(pid,Number(name));break;}
      } catch {}
    }
    if(!hostPids.has(pid))throw new Error('Could not resolve benchmark PID namespace');
  }
  pid=hostPids.get(pid);
  const result={};
  for(const line of fs.readFileSync(`/proc/${pid}/smaps_rollup`,'utf8').split('\n')) {
    const match=line.match(/^(Rss|Pss|Private_Clean|Private_Dirty):\s+(\d+)/);
    if(match)result[match[1]+'_kib']=Number(match[2]);
  }
  return result;
}
async function memory(page,pid) {
  if(await page.evaluate(()=>Boolean(window.__NANO__)))await page.waitForFunction(()=>!document.documentElement.dataset.nanoPending||document.documentElement.dataset.nanoPending==='0');
  const cdp=await page.context().newCDPSession(page);
  await cdp.send('HeapProfiler.collectGarbage');
  const heap=await cdp.send('Runtime.getHeapUsage'),dom=await cdp.send('Memory.getDOMCounters');
  await cdp.detach();
  return {server:serverMemory(pid),browser_heap:heap,dom};
}
const start = performance.now();
const server = spawn(command[0], command.slice(1), {cwd: here, env: {...process.env,
  REFLEX_ENV_MODE: 'prod', NANO_RENDERER: kind.includes('react')?'react':'html', NANO_BENCH_PORT: '3355', NANO_MANIFEST: manifest, REFLEX_SKIP_COMPILE:'true'}, stdio: ['ignore', 'pipe', 'pipe']});
...
(async () => {
  ...
  const server_idle=serverMemory(server.pid);
  ...
  const result = {kind, rows, startup_ms, server_idle, browser: browser.version(), navigations: [], errors: []};
  try {
    for (let repetition = 0; repetition < repetitions; repetition++) {
      ...
      for (const phase of ['initial', 'first_reload']) {
        ...
        const next = await page.evaluate(()=>window.__lifecycle.expectedCount+1);
        ...
        record.memory_hydrated=await memory(page,server.pid);
        if(phase==='initial') {
          record.warm_events=await page.evaluate(async({next,rows})=>{
            const scalar=[];for(let i=1;i<=50;i++)scalar.push(await window.__lifecycle.event('#increment','count',next+i,'doubled',(next+i)*2));
            const toggle=[];
            for(let i=0;i<20;i++)toggle.push(await new Promise((resolve,reject)=>{
              const done=i%2===0,expected=done?rows-1:rows,started=performance.now();
              const timer=setTimeout(()=>{observer.disconnect();reject(new Error('Toggle timing timeout'));},30000);
              const observer=new MutationObserver(()=>{
                if(document.querySelector('#items > .item:last-child .item-done').textContent===String(done)&&document.getElementById('remaining').textContent===String(expected)) {
                  clearTimeout(timer);observer.disconnect();resolve(performance.now()-started);
                }
              });
              observer.observe(document,{subtree:true,childList:true,characterData:true,attributes:true});
              document.querySelector('#items > .item:last-child button').click();
            }));
            sessionStorage.setItem('bench-count',String(next+50));
            return {scalar_ms:scalar,toggle_ms:toggle};
          },{next,rows});
          record.memory_after_updates=await memory(page,server.pid);
        } else await page.evaluate(next=>sessionStorage.setItem('bench-count',String(next)),next);
