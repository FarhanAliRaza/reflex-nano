...
r=json.loads((HERE/'optimized-results.json').read_text())
...
lines=['# WebSocket performance — Reflex Nano 0.3', '',
...
'## What the result means','','The Rust implementation removes Python event execution and avoids copying unchanged state collections. Rust stores each field behind a shared pointer and copies a field when it changes. Computed values reuse shared cached outputs; full snapshots and wire deltas serialize borrowed Rust values. This makes the scalar-event path largely insensitive to the untouched list size in these tests.', '',
...
(HERE.parent/'docs/BACKEND-OPTIMIZED.md').write_text('\n'.join(lines)+'\n')
with (HERE/'optimized-summary.csv').open('w',newline='') as file:
...
print('Wrote docs/BACKEND-OPTIMIZED.md and benchmarks/optimized-summary.csv')
